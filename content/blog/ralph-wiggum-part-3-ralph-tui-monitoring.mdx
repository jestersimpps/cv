...
**Understanding iterations vs tasks:** An iteration is one complete think-act-observe cycle—Claude reads the prompt, takes action (writes code, runs tests), observes results, and corrects if needed. A task from your `IMPLEMENTATION_PLAN.md` may require multiple iterations to complete. For example, implementing authentication might take 8 iterations: initial implementation (2), fixing failing tests (3), adding edge cases (2), and final cleanup (1).

...
Traditional exchanges — whether it's the NYSE or Binance — use **order books**. Think of it like a marketplace noticeboard:
...
In 2018, a developer named Hayden Adams launched **Uniswap** and changed everything. Instead of matching buyers with sellers, Uniswap introduced the **Automated Market Maker (AMM)** — a smart contract that *is* the market.
...
No company. No employees. No downtime. Just a smart contract doing multiplication.
...
Here's the deal: you deposit an equal value of two tokens into a pool. In return, you earn a cut of every single trade that happens in that pool. On Uniswap, that's typically 0.3% of each swap, distributed proportionally to all LPs.
...
Here's the part nobody talks about until it's too late. **Impermanent loss** is the single most important concept for anyone thinking about providing liquidity.
...
The question every LP needs to answer: **Do the trading fees I earn outweigh my impermanent loss?** Sometimes yes, sometimes very much no.

## Concentrated Liquidity: The Uniswap v3 Upgrade

Classic AMMs spread your liquidity across all possible prices — from $0 to infinity. That's wildly inefficient. Most of your capital sits in price ranges that will never be used.

**Uniswap v3** introduced **concentrated liquidity**: you choose the price range where you want to provide liquidity.
...
The tradeoff? If the price moves outside your range, your position stops earning fees entirely and you're left holding 100% of the less valuable token. It's more capital efficient but requires active management.
...
1. **Choose your DEX** — Uniswap (Ethereum), Raydium (Solana), PancakeSwap (BNB Chain), etc.
...
- **Uniswap** — The OG. Ethereum mainnet + L2s (Arbitrum, Optimism, Base). The gold standard
- **Curve Finance** — Specialized in stablecoin and pegged-asset swaps. Extremely low slippage for similar-value tokens
- **PancakeSwap** — The biggest DEX on BNB Chain. Lower fees, sometimes shadier tokens
- **Raydium** — Leading AMM on Solana. Fast, cheap, integrated with Serum's order book
- **Orca** — Another Solana favorite. Clean UX, concentrated liquidity via "Whirlpools"
- **Aerodrome** — The dominant DEX on Base (Coinbase's L2). A Velodrome fork that's become the liquidity hub for the Base ecosystem

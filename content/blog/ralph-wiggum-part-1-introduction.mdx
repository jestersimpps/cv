...
For overnight runs in a sandboxed environment, you can bypass permission prompts entirely:
...
When Ralph runs overnight, you'll want visibility. **Ralph TUI** gives you a real-time dashboard: iteration count, current task, token usage, and keyboard controls to pause or stop.
...
- **Claude Max 20x ($200/mo)** — 200-600+ iterations/session, professional/overnight loops

**My recommendation:** Start with Pro to learn the workflow. Upgrade to Max 5x once you're running Ralph daily—it's 5x the capacity for 5x the price, which is fair. Go Max 20x if you're doing client work or need overnight autonomy.
...
```bash
# Installation
brew install jq                                    # Dependency (macOS)
/plugin install ralph-loop@claude-plugins-official # Plugin

# Basic usage
/ralph-loop "<prompt>" --max-iterations 20 --completion-promise "DONE"

# Safe first experiment
/ralph-loop "Add tests to src/utils.ts until coverage > 80%. \
...
- Always set `--max-iterations` — it's your safety net
...
- [Official Plugin](https://github.com/anthropics/claude-code/tree/main/plugins/ralph-wiggum) — Installation and docs
- [Geoffrey Huntley's Guide](https://github.com/ghuntley/how-to-ralph-wiggum) — Original technique
- [The Ralph Playbook](https://claytonfarr.github.io/ralph-playbook/) — Comprehensive methodology
...
*Next: [Part 2: The Three-Phase Methodology](/blog/ralph-wiggum-part-2-methodology)*

...
argument-hint: "prompt" [output-path] [reference-image]
...
2. Run: node ~/.claude/scripts/generate-image.js "prompt" "./output.png" [reference.png]
...
async function generateImage(prompt, outputPath, referenceImagePath) {
  const requestBody = {
    instances: [{ prompt }],
    parameters: {
      sampleCount: 1,
  ...
  // Add reference image for style/content guidance
...
const [prompt, outputPath = `./generated-${Date.now()}.png`, refImage] = process.argv.slice(2);
generateImage(prompt, outputPath, refImage).then(p => console.log(`Saved: ${p}`));
```
...
/imagen "same style but with a rocket ship" ./output.png ./existing-logo.png
```

**Reference image types:**
- `REFERENCE_TYPE_STYLE` — Match the visual style of the reference
- `REFERENCE_TYPE_SUBJECT` — Include the subject from the reference

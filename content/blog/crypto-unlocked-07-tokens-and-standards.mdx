...
Back in 2015, a developer named Fabian Vogelsteller proposed a simple idea: what if all tokens on Ethereum followed the same basic rules? What if every token could be sent, received, and checked the same way?
...
- **USDT:** Larger market cap, murkier reserves, perpetually controversial — but stubbornly dominant

### Crypto-Collateralized (Decentralized)

**DAI** (MakerDAO) takes a different approach. There's no bank account. Instead, users lock up *more* crypto than the DAI they mint. Want to create 100 DAI? You might need to lock up $150 worth of ETH as collateral. If your collateral drops in value, the system liquidates it automatically.
...
This is what **Terra/UST** tried. For a while, it worked. UST held its peg, and its companion token LUNA soared to $100+. Then in May 2022, it didn't work. UST lost its peg, LUNA entered a death spiral, and **$40 billion evaporated in a week**. People lost their life savings. Some lost more than that.
...
- **ERC-721:** Each token is unique. One token = one distinct item. This is what CryptoPunks and Bored Apes use.
- **ERC-1155:** A hybrid. Can handle both fungible and non-fungible tokens in one contract. Great for games where you might have unique legendary swords *and* stacks of identical health potions.
...
- **Domain names** (ENS gives you a .eth name that *is* an NFT)
...
The JPEGs were the first experiment. They were also the loudest and dumbest. But NFTs as a concept aren't going away — they're just finding their actual use cases.
...
Platforms like **pump.fun** on Solana made it possible to launch a memecoin in under a minute for a few dollars. This spawned an explosion of token launches — thousands per day — most going to zero within hours, a few making early buyers absurdly rich.
...
- **Airdrops:** Free tokens distributed to early users of a protocol. Uniswap's UNI airdrop gave ~$1,500 worth of tokens to anyone who'd used the platform. These can be life-changing — or worthless.

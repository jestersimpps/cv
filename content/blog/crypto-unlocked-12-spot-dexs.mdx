...
In [Part 11](/blog/crypto-unlocked-11-defi-intro) we covered DeFi fundamentals. Now let's zoom into where most of DeFi's action actually happens: **spot DEXs** — the places where you swap one token for another, right now, at market price. There are dozens of them across every chain, each with a different twist. Let's tour the heavyweights.
...
- **V4 (2024):** Added **hooks** — customizable plugins that let developers bolt new logic onto pools. Custom fee structures, on-chain limit orders, dynamic fees that adjust with volatility. It turned Uniswap from a DEX into a DEX *platform*.
- **Unichain (2025):** Uniswap's own Layer 2 chain, purpose-built for trading. Faster blocks, cheaper fees, MEV protection baked in. The DEX got its own country.
...
- **Smart routing:** Your swap might hop through Raydium, Orca, and three other pools in a single transaction to minimize slippage. You don't see any of that complexity — you just get a better price.

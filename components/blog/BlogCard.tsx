...
export default function BlogCard({ post, index, featured = false, viewCount }: BlogCardProps) {
  ...
  return (
    ...
    >
      <Link
        href={`/blog/${post.slug}`}
        className="group flex flex-col bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:bg-white/10 hover:border-white/20 transition-all h-full"
      >
        {post.coverImage && (
